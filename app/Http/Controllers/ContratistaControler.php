...
class ContratistaControler extends Controller
{
    ...
    public function view_create_contratos($id){
        ...
        $centros = Centro::all();
        return view('modulos.gestion_contratistas.crear_contratos', compact("contratista", "procesos", "objetos", "supervisores", "centros"));
    }
        ...
        return DataTables::of($contratos)
            ->editColumn('estado', function($contrato){
                if ($contrato->estado == 1) {
                    $estado = '<div style="padding: 6px; font-size: 13px;" class="badge badge-success">Contrato activo</div>';
                }else{
                    $estado = '<div style="padding: 6px; font-size: 13px;" class="badge badge-danger">Contrato inactivo</div>';
                }
                return $estado;
            })
            ->addColumn('Opciones', function ($contratista) {
                $btn_editar = '<a style="width: 30px;" href="/contratistas/contratos/editar/'.$contratista->id_contratista.'" class="btn btn-sm btn-versatile_reports"><i class="ft-edit"></i></a>';
                $btn_detalles = '<a style="width: 30px;" href="/contratistas/contratos/detalles/'.$contratista->id_contratista.'" class="btn btn-sm btn-gris"><i class="ft-eye"></i></a>';
                if ($contratista->estado == 1) {
                    $btn_estado = '<a style="width: 30px;" href="/contratistas/contratos/cambiar/estado/'.$contratista->id_contratista.'/0" class="btn btn-sm btn-danger btn-estados"><i class="ft-alert-octagon"></i></a>';
                }else{
                    $btn_estado = '<a style="width: 30px;" href="/contratistas/contratos/cambiar/estado/'.$contratista->id_contratista.'/1" class="btn btn-sm btn-success btn-estados"><i class="ft-check-square"></i></a>';
                }
    ...
    public function save_contrato(Request $request){
        ...
        try {
            Contrato::create([
                ...
                'estado' => $request->estado_contrato,
