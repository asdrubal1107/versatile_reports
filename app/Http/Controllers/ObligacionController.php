...
class ObligacionController extends Controller
{
    public function view_list(){
        return view('modulos.parametrizaciones.gestion_obligaciones.listar_obligaciones');
    }

    public function view_create(){
        $procesos = Proceso::all();
        return view('modulos.parametrizaciones.gestion_obligaciones.crear_obligaciones', compact("procesos"));
    }
    
    public function view_edit($id){
        $obligacion = Obligacion::find($id);
        $procesos = Proceso::all();
        return view('modulos.parametrizaciones.gestion_obligaciones.editar_obligaciones', compact("obligacion", "procesos"));
    }
        ...
        if(request()->ajax()){
            ...
            return DataTables::of($obligaciones)
                ->addColumn('Opciones', function ($obligacion) {
                    return '<a style="width: 30px; height: 30px;" href="/parametrizaciones/obligaciones/editar/'.$obligacion->id_obligacion.'" class="btn btn-sm btn-versatile_reports"><i class="ft-edit"></i></a>';
                })
                ->rawColumns(['Opciones'])
                ->make(true);
        }
